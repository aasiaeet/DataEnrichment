...
\label{sec:opt}
We propose \emph{Data Enricher} (DE) a projected block gradient descent algorithm, Algorithm \ref{alg2}, where $\Pi_{\Omega_{f_g}}$ is the Euclidean projection onto the set $\Omega_{f_g}(d_g) = \{f_g(\bbeta) \leq d_g\}$ where $d_g = f_g(\bbeta_g^*)$ and is dropped to avoid cluttering. In practice, $d_g$ can be determined by cross-validation.

\begin{algorithm}[t]
	\caption{  DE: {\sc Data Enricher}}
	\label{alg2}
	\begin{algorithmic}[1]
...
To analysis convergence properties of DE, we should upper bound the error of each iteration.% of the DE algorithm.
Let's $\ddelta^{(t)} = \bbeta^{(t)} - \bbeta^*$ be the error of  iteration $t$ of DE, i.e., the distance from the true parameter (not the optimization minimum, $\hbbe$).
...
\end{corollary}
	...
	Therefore, Corollary \ref{corr:show} shows that DE converges to a scaled variant of statical error bound determined in Corollary \ref{corr:calcub}.
