...
\begin{definition}
	\label{def:only}
	We define the following positive constants for $\mu_g > 0$: %, where for simplification we assume $\X_0 = \oomega$ and $\oomega_0 = \oomega$:
%	\be
%	\nr
...
%	\ee
	\be
	\nr
	\forall g \in [G]&:& \rho_g(\mu_g) = \sup_{\u, \v \in \cB_g} \v^T \big(\I_g - \mu_g \X_g^T \X_g\big) \u, \quad \eta_g(\mu_g) = \mu_g \sup_{\v \in \cB_g} \v^T \X_g^T \frac{\oomega_g}{\norm{\oomega_g}{2}}
	\\ \nr
...
Therefore, if we can keep $\rho$ below one, the estimation error of PBGD algorithm geometrically converges to the approximate statistical error bound. %and we get to $\hbbe$ exponentially fast.
...
One way for having $\rho < 1$ is to keep all arguments of $\max(\cdots)$  determining $\rho$ strictly below $1$. %, i.e., $\rho_g < 1/\theta_f $.
...
\begin{theorem}
	...
	\nr
%	&\theta_f = 1:&
	\mu_0 = \frac{1}{4	n} \times \min_{g \in [G]_\setminus} \left(1 + c_{0g} \frac{\omega_{0g}+ \tau}{\sqrt{n_g}}\right)^{-2}, \quad \text{and} \quad \mu_g =  \frac{1}{2\sqrt{n n_g}} \left(1 + c_{0g}\frac{\omega_{0g}+ \tau}{\sqrt{n_g}} \right)^{-1}
%	\\ \nr
...
%	where $0 < l < 1$ is arbitrary,
	...
	\nr
	\sum_{g=0}^{G} \sqrt{\frac{n_g}{n}} \norm{\ddelta_g^{(t+1)}}{2}
	\leq r(\tau)^t \sum_{g=0}^{G} \sqrt{\frac{n_g}{n}} \norm{\bbeta^*_g}{2}   + \frac{(G+1) \sqrt{(2K^2 + 1)}}{\sqrt{n} (1 - r(\tau))}  \left(\zeta k \max_{g \in [G]} \omega(\cA_g) + \tau \right),
	\ee
...
\begin{corollary}
	...
	\leq \frac{(G+1) \sqrt{(2K^2 + 1)}}{\sqrt{n} (1 - r(\tau))}  \left(\zeta k \max_{g \in [G]} \omega(\cA_g) + C \sqrt{\log (G+1)} +  b \right),
	\ee
\end{corollary}
