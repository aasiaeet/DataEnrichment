...
Note that each of the linear models of \eqref{eq:dsl} is a superposition \cite{guba16} or dirty statistical model \cite{yara13}. Therefore, a data-enriched model is \emph{a system of coupled superposition models}.
...
Also beyond sparsity and $l_1$-norm, no other structure has been investigated for the data enriched model. Moreover, no computational results, such as rates of convergence of the proposed estimators, exist in the literature.
...
%	So the common parameter's estimator exploits all of the pooled data to reduce its error.
	...
	\item We present an efficient Projected Block Gradient Descent (PBGD) algorithm to solve DE's objective \eqref{eq:super} which converges linearly to the statistical error bound of \eqref{eq:errorsum}. To the best of our knowledge, this is the first rigorous computational result for the high-dimensional data-enriched regression.
	\item Finally, we apply our algorithm PBGD to find bio-markers associated with drug sensitivity of cell lines from different cancer types. The support of individual parameters $\hbbe_g^T$ estimated by PBGD for each cancer $g$ highlights a different set of bio-markers per cancer type.
\end{itemize}
