...
\label{realexp}
In this section, we investigate the application of DE in analyzing the response of cancer tumor cell lines to different doses of various drugs. 
...
Figure \ref{fig:two} and \ref{fig:three} illustrate the performance of DE and BL for TWO and THREE experiments respectively. 
Note that DE outperforms BL in 21 and 18 out of 24 problems in TWO and THREE experiments, respectively.

To ensure that the prediction improvement of DE over the baseline is significant, we supplement our analysis with the bootstrapped error of both methods for the TWO experiment.
...
Then, we fix $d_g$s hyper-parameters to values determined by cross-validation in the last stage and run both methods and compute pairs of MSEs as $\{(\text{MSE}^{(i)}_{\text{DE}}, \text{MSE}^{(i)}_{\text{BL}})\}_{i=1}^{100}$. We perform paired t-test to determine if mean difference between MSEs is significant. In ? out of 24 problems DE's MSE is lesser than BL's with significance level of $\alpha = 0.05$. A representative set of results is demonstrated in Figure \ref{?}. 
	...
	\end{subfigure}
	\caption{Comparison of Mean Square Error of DE and BL in predicting the response to 24 drugs in TWO and THREE experiments. Each bar is the mean of MSE for 5-fold cross-validation.}
	\label{fig syn2}
\end{figure}
{\bf Interpretation}
