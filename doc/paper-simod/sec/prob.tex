...
%Beside a deterministic error bound presented in Section \cref{sec:deter} our other results are probabilistic.
...
A compact form of our proposed DE estimator \cref{eq:super} is:% optimization problem as: %:
\beq
\label{eq:compact}
...
{\bf ($l_1$-norm)} When all parameters $\bbeta_g$s are $s_g$-sparse, i.e.,$|\text{supp}(\bbeta_{g}^*)| = s_g$ by using $l_1$-norm as the sparsity inducing function, our DE estimator of \cref{eq:compact} becomes:
\be 
\hbbe \in \argmin_{\bbeta } \frac{1}{n} \norm{\y - \X \bbeta }{2}^2, \quad \text{s.t. }  \forall g \in [G_+]: \norm{\bbeta_g}{1} \leq \norm{\bbeta^*_g}{1}.
\ee 
We call \cref{exm:sde} \emph{sparse DE } estimator and use it as the running example throughout the paper to illustrate outcomes of our analysis.
\end{example}
