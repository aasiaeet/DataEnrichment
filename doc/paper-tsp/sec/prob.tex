...
\label{sec:esti}
A compact form of our proposed DS estimator \eqref{eq:super} is:% optimization problem as: %:
{\small \beq
\label{eq:compact}
\hbbe \in \argmin_{\bbeta } \frac{1}{n} \norm{\y - \X \bbeta }{2}^2, \text{s.t. } \forall g \in [G_+]:f_g(\bbeta_g ) \leq f_g(\bbeta^*_g),
\eeq }
...
\be \nr 
\hbbe \in \argmin_{\bbeta } \frac{1}{n} \norm{\y - \X \bbeta }{2}^2, \text{s.t. }  \forall g \in [G_+]: \norm{\bbeta_g}{1} \leq \norm{\bbeta^*_g}{1}.
\ee 
We call \eqref{exm:sde} \emph{sparse DS} estimator and use it as the running example throughout the paper to illustrate outcomes of our analysis.% \hfill {\color{header1} \qedsymbol}
\end{example}
...
\beq
\nr 
\frac{1}{G} \sum_{g=0}^G \| \delta_g \|_2 \leq O( G^{1/2} ) \frac{\sup_{\u \in \bcH}\w^T \X \u}{n}~.
\eeq
\end{remark}
